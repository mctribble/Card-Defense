...
public class EditorBalanceTesterScript : BaseBehaviour
{
    ...
    private float DPS_10_range(PlayerCardData pcd) { return Mathf.Min(maxTargets(pcd), 10 * pcd.towerData.range) * DPS_1(pcd); }    //damage per second the given tower card does, if it has 10*range enemies in range
    ...
    private float DPS_10_range(TowerScript tower) { return Mathf.Min(maxTargets(tower), 10 * tower.range) * DPS_1(tower); }      //damage per second the given tower card does, if it has 10*range enemies in range
